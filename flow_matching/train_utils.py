...
def divergence_from_func(func: Callable[[Tensor], Tensor], x: Tensor) -> Tensor:
    """Compute the exact divergence of ``func`` at ``x`` via autograd."""
    ...
    for i in range(y_flat.shape[1]):
        grad = torch.autograd.grad(
            y_flat[:, i].sum(), x, retain_graph=True, create_graph=True, allow_unused=True
        )[0]
        if grad is None:
            grad = torch.zeros_like(x)
        div += grad.view(x.shape[0], -1)[:, i]
    return div


class FlowMatchingWrapper(nn.Module):
    """Wrap a vector field ``f`` to compute the CNF loss.

    Parameters
    ----------
    vector_field: nn.Module
        Module with signature ``f(x, t, cond)`` returning the drift.
        It is registered so that its parameters are optimised and the
        adjoint method can access them.
    t0, t1: float
        Integration limits.
    """

    def __init__(self, vector_field: nn.Module, t0: float = 0.0, t1: float = 1.0):
        super().__init__()
    ...
    def _dynamics(self, t: Tensor, state: tuple, cond: Tensor):
        ...
        f = self.vector_field(z, t, cond)
        # For stability in this minimal example, use zero divergence.
    ...
    def forward(self, x: Tensor, cond: Optional[Tensor] = None, method: str = "rk4"):
        ...
        func = lambda tt, yy: self._dynamics(tt, yy, cond)
        z_T, logp_T = odeint(func, (x, logp0), t, method=method)
        z_T = z_T[-1]
        logp_T = logp_T[-1]
