...
def main() -> None:
    ...
    if args.mode == "sample":
        ...
        while True:
            try:
                tmp = IQBurstDataset(str(root))
                tmp.cond_for_path(str(file_path))
            except ValueError:
                pass
            else:
                if tmp.cond_dim == cond_dim:
                    dataset = tmp
                    break
            if root.parent == root:
                break
            root = root.parent
        if dataset is None:
            raise ValueError("Could not infer dataset root; ensure file is inside training dataset")
        arr = np.load(file_path)
        arr = np.stack([arr.real, arr.imag], axis=-1).astype(np.float32)
        cond = dataset.cond_for_path(str(file_path)).unsqueeze(0).to(device)
